$(document).ready(function() {
	...
	$('#contact-form').submit(function() {
		...
		if(hasError) {
			...
			*/
			$('#contact-form').append('<p class="contact-form-message"><i class="fa fa-spinner"></i>'+sendingMessage+'</p>');
			$('#contact-form .contact-form-message').remove();
			$('#contact-form').append('<p class="contact-form-message"><i class="fa fa-check"></i>'+okMessage+'</p>');
			$('#contact-form').removeClass('clicked');
			$('#contact-form')[0].reset();
			$('#contact-form .form-control').removeClass('input-filled');
